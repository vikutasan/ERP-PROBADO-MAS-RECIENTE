...
async def migrate():
    async with engine.begin() as conn:
        ...
        table_check = await conn.execute(text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'product_technical_sheets')"))
        if not table_check.scalar():
            print("Notice: Table 'product_technical_sheets' does not exist yet. Aborting migration gracefully.")
            return
