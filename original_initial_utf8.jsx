...
// Importaci├│n de m├│dulos creados (Simulada para el demo)
...
/**
 * R DE RICO - CENTRO DE EXPERIMENTACI├ôN (DASHBOARD MAESTRO)
 * 
...
export const ExperimentCenterUI = () => {
    ...
    const allModules = [
        { id: 'pos_retail', name: 'Punto de Venta IA', color: 'bg-orange-600', icon: '­ƒøÆ', access: ['ADMIN', 'MANAGER'] },
        { id: 'inventory', name: 'Gesti├│n de Productos', color: 'bg-indigo-600', icon: '­ƒôª', access: ['ADMIN', 'MANAGER'] },
        { id: 'warehouse', name: 'Gesti├│n de Almacenes', color: 'bg-slate-700', icon: '­ƒÅ¼', access: ['ADMIN', 'MANAGER'] },
        { id: 'vision_train', name: 'Entrenamiento IA', color: 'bg-[#c1d72e]', icon: '­ƒæü´©Å', access: ['ADMIN', 'MANAGER'] },
        { id: 'production', name: 'Maestro Panadero', color: 'bg-amber-800', icon: '­ƒì×', access: ['ADMIN', 'BAKER'] },
        { id: 'financials', name: 'M├│dulo Financiero', color: 'bg-emerald-800', icon: '­ƒôê', access: ['ADMIN', 'MANAGER'] },
        { id: 'invoicing', name: 'Facturaci├│n CFDI', color: 'bg-blue-600', icon: '­ƒº¥', access: ['ADMIN', 'MANAGER', 'CASHIER'] },
        { id: 'purchasing', name: 'Gesti├│n de Compras', color: 'bg-indigo-900', icon: '­ƒøÆ', access: ['ADMIN', 'MANAGER'] },
        { id: 'procurement', name: 'B2B Procurement', color: 'bg-blue-900', icon: '­ƒñØ', access: ['ADMIN', 'MANAGER'] },
        { id: 'logistics', name: 'Log├¡stica de Rutas', color: 'bg-blue-600', icon: '­ƒÜÜ', access: ['ADMIN', 'MANAGER', 'LOGISTICS'] },
        { id: 'pos_tables', name: 'TPV Mesas & KDS', color: 'bg-orange-600', icon: '­ƒì¢´©Å', access: ['ADMIN', 'MANAGER', 'WAITER'] },
        { id: 'waiter', name: 'App Mesero', color: 'bg-rose-700', icon: '­ƒô▒', access: ['ADMIN', 'WAITER'] },
        { id: 'driver', name: 'App Repartidor', color: 'bg-gray-800', icon: '­ƒô▒', access: ['ADMIN', 'DRIVER', 'LOGISTICS'] },
        { id: 'seguridad_acceso', name: 'Seguridad y Acceso', color: 'bg-indigo-900', icon: '­ƒöæ', access: ['ADMIN', 'MANAGER'] },
        { id: 'auditoria', name: 'Auditor├¡a y Control', color: 'bg-slate-900', icon: '­ƒôï', access: ['ADMIN', 'MANAGER'] },
        { id: 'settings', name: 'Ajustes del Sistema', color: 'bg-red-900', icon: 'ÔÜÖ´©Å', access: ['ADMIN'] },
    ];
    ...
    const visibleModules = allModules.filter(mod => {
        ...
        // Si el usuario tiene una configuraci├│n de permisos granulares (objeto no vac├¡o),
        ...
        // Solo si no hay permisos granulares cargados, usamos la l├│gica de roles legacy.
    ...
    const mockData = {
        pendingDeliveries: [
            { id: 'ORD-001', customerName: 'Restaurante Gaucho', deliveryAddress: 'Av. Ju├írez 45, Toluca', deliveryDate: new Date(), status: 'READY' },
        ...
        drivers: [{ id: 'D1', firstName: 'Juan', lastName: 'P├®rez' }],
        dailyPlan: [
        ...
        clients: [
            ...
            { id: 'C2', restaurantName: 'Pizzer├¡a Napolitana', contactName: 'Mario Rossi', phone: '722-987-6543', address: 'Metepec' }
        ]
    };
        ...
        <div className="bg-[#050505] h-screen text-white font-sans flex overflow-hidden">
            ...
            <aside className={`${isSidebarCollapsed ? 'w-20' : 'w-80'} bg-black/40 border-r border-gray-800 p-8 flex flex-col backdrop-blur-3xl transition-all duration-500 ease-in-out relative group`}>
                ...
                <nav className="flex-1 space-y-2 overflow-y-auto overflow-x-hidden pr-2 custom-scrollbar">
                    ...
                    >
                        {isSidebarCollapsed ? '­ƒÅá' : '­ƒÅá Vista General'}
                    </button>
                ...
                >
                    {isSidebarCollapsed ? 'Ô£û' : 'Ô£û Cerrar Sesi├│n'}
                </button>
            </aside>
            ...
            >
                ...
                <div className="relative z-10 h-full">
                        ...
                        <div className="p-20 max-w-6xl mx-auto space-y-20 animate-in fade-in zoom-in-95 duration-700">
                            <section>
                                ...
                                <p className="text-2xl text-gray-400 font-medium max-w-2xl leading-relaxed">
                                    Socio, has construido una arquitectura de nivel mundial. Selecciona un m├│dulo en el panel izquierdo para vivir la experiencia de **R de Rico**.
                                </p>
                            </section>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="p-10 bg-gray-900/40 rounded-[50px] border border-gray-800">
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-center"><span className="text-gray-500 font-bold italic">Producci├│n hoy:</span> <span className="font-black">100%</span></div>
                                ...
                                <div className="p-10 bg-gray-900/40 rounded-[50px] border border-gray-800">
                                    <h3 className="text-2xl font-black uppercase mb-4 text-blue-400">Log├¡stica</h3>
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex justify-between items-center"><span className="text-gray-500 font-bold italic">Veh├¡culos en Campo:</span> <span className="font-black text-blue-500">2</span></div>
                    ...
                    <div className="h-full">
                        ...
                        {activeModule === 'driver' && <DriverAppUI activeRoute={{ orders: [] }} currentDriver={{ name: 'Juan P├®rez' }} />}
